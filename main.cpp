...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	int backGround = Novice::LoadTexture("./bg1.png");
	...
	while (Novice::ProcessMessage() == 0) {
		...
		case STAGE1:
			...
			screen.posx -= screen.speedx;//背景
			if (screen.posx < -1280) {//画面外に行ったらx1280に代入
				screen.posx = 0;
				screen.posy = 0;
			}
		...
		case STAGE1:
			...
			//スクリーン
			Novice::DrawSprite(screen.posx, screen.posy, backGround, 1, 1, 0.0f, WHITE);
			Novice::DrawSprite(screen.posx + 1280, screen.posy, backGround, 1, 1, 0.0f, WHITE);

			Novice::DrawSprite((int)player.position.x - (int)player.radius.x, (int)newposY - (int)player.radius.y, sample, 1, 1, 0.0f, WHITE);
		...
		case BOSS:
			...
			Novice::DrawSprite((int)player.position.x - (int)player.radius.x, (int)newposY - (int)player.radius.y, sample, 1, 1, 0.0f, WHITE);
			Novice::DrawBox((int)player.position.x - (int)player.radius.x, (int)player.position.y - (int)player.radius.y, (int)player.position.x, (int)player.position.y, 0.0f, WHITE, kFillModeSolid);
